# Past Questions

## 1 Introduction (4 hours)

### 1.1 Software and software engineering
- Explain how modern software engineering practices helped overcome the software crisis. [3 marks] (2082 Bhadra)

### 1.2 Nature and characteristics of software
- Define software and list typical software characteristics. [4 marks] (2078 Bhadra)
- What are the characteristics of good software? [2 marks] (2081 Bhadra)
- Justify: "Software doesn't wear out like hardware components." [5 marks] (2082 Bhadra)

### 1.3 Software application domains
- (No direct past questions found)

### 1.4 Legacy software
- (No direct past questions found)

### 1.5 Software crisis
- What is software crisis? Explain with an example. [5 marks] (2068 Chaitra)
- What factors have contributed to the software crisis? Suggest possible solutions. [3+3 marks] (2075 Ashwin)

### 1.6 Software myths
- (No direct past questions found)

### 1.7 Software engineering practice
- (No direct past questions found)

---

## 2 The Software Process (8 hours)

### 2.1 Process framework and umbrella activities
- Explain fundamental activities of the software process. [3 marks] (2082 Bhadra)

### 2.2.1 Waterfall model and its extensions
- What is SDLC? Discuss the various stages of the waterfall process model. [2+3 marks] (2081 Baishakh)
- Discuss the advantages and disadvantages of the waterfall model. [4 marks] (2079 Bhadra)
- Explain why the waterfall model is not suitable when important functionalities need to be delivered in a short time period. [4 marks] (2073 Chaitra)

### 2.2.2 Incremental process model
- Explain the incremental model with its advantages and disadvantages. [4+3 marks] (2075 Chaitra)
- You are leading development of an e-commerce site using the incremental model. How do you implement it? [4 marks] (2082 Bhadra)

### 2.2.3 Evolutionary process models (Prototyping, spiral)
- Explain the prototyping model with its advantages and disadvantages. [5+2 marks] (2076 Chaitra)
- In what types of projects can the prototype process model be used? Explain with an example. [5 marks] (2082 Baishakh)
- Explain the spiral model with its advantages and disadvantages. [5 marks] (2068 Chaitra)
- How can both the waterfall model and prototyping model be accommodated in the spiral process model? [6 marks] (2078 Kartik)
- A travel agency needs software but is unsure about the UI. Would it be proper to use the spiral model? Justify. [6 marks] (2075 Ashwin)

### 2.3.1 Agile manifesto and the 12 principles
- (No direct past questions found — new syllabus topic)

### 2.3.2 Agile versus plan-driven development
- (No direct past questions found — new syllabus topic)

### 2.3.3 Scrum framework (Roles, artifacts, ceremonies)
- (No direct past questions found — new syllabus topic)

### 2.3.4 Extreme programming (XP) practices
- (No direct past questions found — new syllabus topic)

### 2.3.5 Lean software development
- (No direct past questions found — new syllabus topic)

### 2.4 Model selection considerations
- Why are different process models used in software development? [2 marks] (2080 Bhadra)
- Define software process model. Differentiate the spiral model and incremental model highlighting their advantages and disadvantages. [2+6 marks] (2080 Baishakh)
- A client's problem has uncertainties that could lead to loss if not planned. Which model do you suggest? Justify. [5 marks] (2074 Chaitra)
- Why do we need to do a feasibility study before accepting any project? [2 marks] (2082 Bhadra)

---

## 3 Software Requirements Engineering (6 hours)

### 3.1 Requirement engineering process
- What is requirement engineering? Explain its steps. [4 marks] (2074 Chaitra)
- What do you mean by a software requirements document? [2 marks] (2078 Bhadra)
- Explain the requirement engineering process in detail. [6 marks] (2078 Bhadra)
- Why is it so difficult to gain a clear understanding of what the customer wants? [3 marks] (2081 Bhadra)
- What are the different techniques used for requirements gathering and analysis? Explain any three methods in detail. [7 marks] (2070 Ashad)

### 3.2 SRS (Structure, characteristics, users)
- What is the importance of the SRS document in software development? [3 marks] (2082 Baishakh)
- Identify and document functional as well as non-functional requirements for "issuing a book from a library." [5 marks] (2079 Bhadra)

### 3.3 Functional and non-functional requirements
- Explain functional and non-functional requirements with examples. [6 marks] (2069 Ashad)
- Differentiate functional and non-functional requirements. [7 marks] (2069 Chaitra)
- Distinguish between user and system requirements. [2 marks] (2079 Bhadra)
- Why should an engineer ensure that functional and non-functional needs are in a requirements specification document? [4 marks] (2079 Bhadra)
- List and explain functional (any two) and non-functional requirements of an airlines reservation system. [6 marks] (2082 Baishakh)
- Explain functional (Order Food, Make Payment) and non-functional (all) requirements of an online food ordering system. [6 marks] (2081 Bhadra)
- Prepare functional requirements for an online ticket booking system for a movie theatre. [4 marks] (2082 Bhadra)
- List functional and non-functional requirements for a Restaurant Information System. [5 marks] (2068 Chaitra)

### 3.4 Gathering requirements using use cases modeling and scenarios
- Prepare a use case diagram for an event management system. [4 marks] (2082 Bhadra)
- Draw a use case diagram for an online food ordering system. [5 marks] (2082 Baishakh)
- Draw a use case diagram for an online appointment booking app. [5 marks] (2081 Bhadra)
- Prepare use case diagrams for an automated ticket issuing system. [5 marks] (2081 Baishakh)
- Draw a use case diagram illustrating interactions between a doctor, patients, and prescriptions. [5 marks] (2078 Bhadra)

### 3.5 Agile requirements engineering
- (No direct past questions found — new syllabus topic)

---

## 4 Architectural Design (3 hours)

### 4.1 Introductions and importance
- What is architectural design? [2 marks] (2082 Baishakh)
- Why is architectural design important in software engineering? [2 marks] (2076 Ashwin)
- Explain why it may be necessary to design the system architecture before specifications are written. [3 marks] (2075 Ashwin)

### 4.2 Architectural design principles
- (No direct past questions found — new syllabus topic)

### 4.3 Taxonomy of architectural styles
- Explain client-server architecture with a suitable example. [5 marks] (2082 Baishakh)
- Describe layered architecture for software with an example. [4 marks] (2082 Bhadra)
- Differentiate repository model and layered model with their advantages and disadvantages. [3+3 marks] (2080 Bhadra)
- Explain multi-tier (2-tier and 3-tier) architecture with examples. [4 marks] (2078 Bhadra)

### 4.4 Modular design, cohesion and coupling
- What are the different modular decomposition styles used during system design? Explain with examples. [5 marks] (2075 Chaitra)
- How is the modular decomposition concept practiced in the system design process? [4 marks] (2071 Shrawan)

---

## 5 System Modeling (9 hours)

### 5.1 System modeling
- Why is system modeling important? [2 marks] (2068 Chaitra)

### 5.2 Process modeling using DFD
- Design Level-0 and Level-1 DFD for an Examination Management System. [5 marks] (2082 Bhadra)
- Draw Level-0 and Level-1 DFD of a Student Attendance Management System. [6 marks] (2082 Baishakh)
- Design Level-0 and Level-1 DFD for an Airline Ticketing System. [7 marks] (2081 Bhadra)
- Develop a Context diagram and Level-1 DFD for an online food ordering system. [3+5 marks] (2081 Baishakh)
- Draw Level-0 and Level-1 DFD for an online shopping system. [3+5 marks] (2079 Bhadra)

### 5.3 Scenario-based analysis (Use-case descriptions and diagrams)
- (Use-case diagram questions listed under 3.4 above)

### 5.4.1 Activity diagrams
- (No direct past questions found — new syllabus topic)

### 5.4.2 Class-based modeling
- (No direct past questions found — new syllabus topic)

---

## 6 Coding and Testing (5 hours)

### 6.1 Coding standards and guidelines
- (No direct past questions found — new syllabus topic)

### 6.2 Code review (walkthrough, inspection, cleanroom)
- What is a software inspection process? [4 marks] (2082 Bhadra)
- What are the faults that can be uncovered by software inspection? [2 marks] (2082 Baishakh)
- Why are program inspections an effective technique for discovering errors? [5 marks] (2068 Chaitra)
- What types of errors are unlikely to be discovered through inspections? [5 marks] (2068 Chaitra)

### 6.3.1 Verification and validation
- Differentiate between verification and validation. [3 marks] (2082 Baishakh)
- Why is verification and validation planning necessary? [2 marks] (2075 Chaitra)

### 6.3.2 Unit testing
- Explain unit testing. [2 marks] (2078 Kartik)
- Why is unit testing alone not enough for verifying a complex system? [4 marks] (2070 Ashad)

### 6.3.3 Integration testing
- Explain integration testing. [2 marks] (2078 Kartik)
- Write about stub and driver testing. [3 marks] (2075 Chaitra)

### 6.3.4 System testing
- Explain system testing. [2 marks] (2078 Kartik)
- Differentiate between system testing and integration testing. [2 marks] (2081 Baishakh)

### 6.3.5 Acceptance testing
- Differentiate between alpha testing and beta testing. [2 marks] (2081 Bhadra)

### 6.4 Black-box and white-box testing approach
- Differentiate between black-box testing and white-box testing. [5 marks] (2076 Chaitra)
- Are both black-box and white-box testing necessary, or is just one sufficient? Justify. [3 marks] (2079 Bhadra)
- What is equivalence partitioning? Explain with an example. [5 marks] (2070 Ashad)

### 6.5 Agile testing practices (TDD, continuous testing, automated unit testing, refactoring)
- (No direct past questions found — new syllabus topic)

---

## 7 Software Quality, Assurance, Maintenance (4 hours)

### 7.1 Quality concepts
- Define software quality and briefly describe the rationale for your definition. [2+3 marks] (2073 Shrawan)
- What are software quality measures? [2 marks] (2074 Chaitra)

### 7.2 Quality attributes
- Explain quality attributes of software. [3 marks] (2073 Shrawan)

### 7.3 Reviews, inspections and QA concepts
- Define SQA. [2 marks] (2080 Bhadra)
- What is FTR? [2 marks] (2076 Chaitra)
- How is a Formal Technical Review (FTR) conducted? [5 marks] (2078 Bhadra)
- What is the significance of "the vital few" in statistical SQA? [2 marks] (2082 Bhadra)

### 7.4 ISO standards, CMMI levels
- Describe the staged CMMI model with levels and key process areas. [5 marks] (2082 Bhadra)
- Explain different levels of CMMI. [5 marks] (2082 Baishakh)
- How are CMM standards different from ISO standards for software quality? [4 marks] (2073 Chaitra)

### 7.5 Software maintenance and its types
- Describe different types of software maintenance. [10 marks] (2070 Chaitra)

### 7.6 Maintenance effort and lifecycle considerations
- (No direct past questions found — new syllabus topic)

---

## 8 Software Configuration Management (3 hours)

### 8.1 Software configuration management
- Define configuration management and explain SCM activities. [4 marks] (2080 Bhadra)
- Explain why we need Software Configuration Management (SCM). [4 marks] (2073 Shrawan)

### 8.2 Version control and continuous integration
- Define version, variants, and release with respect to configuration management. [4 marks] (2079 Bhadra)
- Differentiate version and variant in the context of configuration management. [2 marks] (2082 Bhadra)

### 8.3 Change management process
- Describe the change management process in software engineering. [4 marks] (2078 Bhadra)

### 8.4 Branching strategies (Git-flow basics)
- (No direct past questions found — new syllabus topic)

---

## 9 Recent Trends (3 hours)

### 9.1 DevOps and continuous deployment pipelines
- (No direct past questions found — new syllabus topic)

### 9.2 CI/CD in modern projects
- (No direct past questions found — new syllabus topic)

### 9.3 Cloud-native, microservices architectures and IaC
- (No direct past questions found — new syllabus topic)

### 9.4 AI-assisted software development
- (No direct past questions found — new syllabus topic)

### 9.5 Low-code/no-code, green software engineering
- (No direct past questions found — new syllabus topic)

---
